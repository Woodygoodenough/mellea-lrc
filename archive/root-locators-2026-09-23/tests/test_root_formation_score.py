...
def test_score_joins_annotation_to_recorded_source_path(tmp_path: Path) -> None:
    annotations = tmp_path / "annotations"
    artifacts = tmp_path / "artifacts"
    annotations.mkdir()
    artifacts.mkdir()
    ...
    _write_json(
        artifacts / "001__example.json",
        {
            "source_metadata": {"path": "corpus/001__example-.txt"},
            "citations": [_citation("p1", "p1", "FullCaseCitation", 10, 20)],
        },
    )

    score = score_root_formation(annotations=annotations, artifacts=artifacts)

    assert score["root_locators"]["tp"] == 1
    assert score["root_locators"]["fp"] == 0
    assert score["root_locators"]["fn"] == 0
    assert score["locator_attribution"]["tp"] == 1
    assert score["locator_attribution"]["fp"] == 0
    assert score["locator_attribution"]["fn"] == 0
