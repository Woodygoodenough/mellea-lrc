...
def run_exact_locator_lookup(
    ...
    *,
    ...
    reporter = str(citation.reporter) if citation.reporter is not None else None
    page = citation.page
    ...
    # CourtListener normally signals several matching opinion clusters with
    # HTTP 300, but live responses also use HTTP 200 with the same plural
    # ``clusters`` payload.  HTTP status therefore cannot safely decide that a
    # locator is unique.  The number of returned clusters is the actual
    # identity boundary: preserve every candidate and send it to the bounded
    # ambiguity stage whenever that number is greater than one.
    if lookup.status in {HTTP_OK, HTTP_MULTIPLE_CHOICES} and len(lookup.clusters) > 1:
        return ExactLocatorLookupNode(
            node_id=node_id,
            status=ValidationNodeStatus.SUCCEEDED,
