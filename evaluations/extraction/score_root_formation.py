...
def score_root_formation(*, annotations: Path, artifacts: Path) -> dict[str, object]:
    ...
    predicted_edges = _predicted_edges(artifacts)
...
def _gold_edges(annotations: Path, *, document_names: set[str]) -> set[_RootEdge]:
    ...
    for path in sorted(annotations.glob("*.jsonl")):
        ...
        locator_by_id = {
            str(row["id"]): _annotation_key(path.stem, row)
            for row in rows
            if _is_root_locator(row)
        }
        for row in rows:
...
def _predicted_edges(artifacts: Path) -> set[_RootEdge]:
    ...
    for path in sorted(artifacts.glob("*.json")):
        ...
        if not isinstance(citations, list):
            ...
            raise ValueError(msg)
        locator_by_id = {
            str(citation["citation_id"]): _prediction_key(path.stem, citation)
        ...
        for citation in citations:
            if not _is_predicted_root_locator(citation):
                continue
            occurrence = _prediction_key(path.stem, citation)
...
def _is_root_locator(row: dict[str, Any]) -> bool:
    return row.get("unit") == "citation" and row.get("kind") in _ROOT_KINDS and isinstance(row.get("locator"), dict)
...
def _is_predicted_root_locator(citation: dict[str, Any]) -> bool:
    ...
    return (
        ...
        and source.get("citation_type") in _ROOT_KINDS
...
def _prediction_key(document: str, citation: dict[str, Any]) -> _LocatorKey:
    ...
    return document, str(source["citation_type"]), int(locator["start"]), int(locator["end"])
