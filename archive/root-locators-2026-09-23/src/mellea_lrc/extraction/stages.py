...
from mellea_lrc.model.operations import assign_antecedent
...
def _root(text: str, citations: Sequence[CitationRecord]) -> tuple[CitationRecord, ...]:
    ...
    for citation in citations:
        ...
        if leader != citation.citation_id:
            node = Node(
                node_id=f"docket_antecedent:{citation.citation_id}:{leader}",
                reads=Reads.RECORD,
                stage="root_assignment",
                made_by=__name__,
                outcome="linked",
                details={"antecedent_id": leader},
            )
            linked.append(assign_antecedent(citation, leader, node))
        else:
            linked.append(citation)
...
def audit_dockets(document: Document, rules: ExtractionRules | None = None) -> Document:
    ...
    if rules is None or rules.docket_auditor is None:
        return document
    audited_citations = tuple(rules.docket_auditor(document.text, document.citations))
    # The public stage can also be replayed after leaf growth. Move affected
    # leaves before constructing the next valid graph checkpoint.
    from mellea_lrc.extraction.structure.withdrawal import withdraw_leaves_of_withdrawn_roots

    withdraw_leaves_of_withdrawn_roots(audited_citations)
    return document.evolve(citations=audited_citations)
...
def resolve_case_names(document: Document, rules: ExtractionRules | None = None) -> Document:
    ...
    return document.evolve(citations=citations, passes=(*document.passes, CASE_NAME_STAGE))
...
def resolve_courts(document: Document, rules: ExtractionRules | None = None) -> Document:
    ...
    return document.evolve(citations=citations, passes=(*document.passes, COURT_STAGE))
...
def resolve_dates(document: Document, rules: ExtractionRules | None = None) -> Document:
    ...
    return document.evolve(citations=citations, passes=(*document.passes, DATE_STAGE))
