...
Start a `Document` from text or a file, then invoke the root-growth stage:

```python
from pathlib import Path
from mellea_lrc.api import Document, grow_roots

text_document = Document.from_source("See Brown v. Board of Education, 347 U.S. 483, 495 (1954).")
text_document = await grow_roots(text_document)

file_document = Document.from_source(Path("filing.pdf"))
file_document = await grow_roots(file_document)
```

`Document.from_source` treats a `str` as content and a `Path` as a file
location. Each named stage takes and returns a `Document`, so the result can be
serialized and resumed at a later stage without rerunning extraction.
...
Callers with a preprocessed result can use `Document.from_preprocessed(...)`.
`PreprocessedDocument` is also a Pydantic model and can cross a process
boundary through its native dump and validation methods.
...
A `Document`, which is the `PreprocessedDocument` it was built from
...
`document.model_dump(mode="json")` writes the whole `Document` as a JSON-ready
mapping, and `Document.model_validate(payload)` reads it back. That artifact is the
...
It is versioned. The current `schema_version` is **21**, and a reader refuses
an incompatible payload. `locators` and `colocations` are derived from the
saved citation records; they are not redundant top-level checkpoint fields.
...
| `operations` | ordered creation, field updates, links, resolutions, and judgements with evidence pointers |
...
The current fields are a direct projection of the append-only operations. Each
operation stores its resulting value and the node that justified it; the prior
value can be recovered from earlier operations.
...
`document.locators` and `document.colocations`; a reloaded checkpoint derives
them from its saved citation records.
...
document = await grow_roots(document, rules=stable(relaxation=Relaxation.FULL))
```
...
### The record: current fields and the evidence for each reading
...
The citation the pipeline holds is the latest reading. Earlier readings are
explicit field updates, rather than a second citation-shaped object.
:class:`~mellea_lrc.model.record.CitationRecord` holds both:

```
CitationRecord
├── citation_id                  never changes
├── fields:  Citation            the citation as currently read
├── operations                  every durable change, in order
├── field_updates               field-only view of operations
├── found:   Resolution | None   what an archive holds        (validation's)
├── root_id / authority_id       what extraction read / what a lookup found
└── trace:   tuple[Node, ...]    evidence supporting durable state
```

**Creation fixes only citation kind.** A document-reading node creates the
record, and the first locator is written through `update_fields`, just like a
later revision. A leaf receives its root link separately before admission.

**An update points to its evidence.** Each `CitationOperation` stores its
before and after values and `node_id`; field changes also store a reason.
One node can justify several operations or no change. Current state is directly
on the record, and the serialized operation sequence preserves its history
without trace traversal.

**The filing and the archive stay separate.** `fields` says what the filing is
currently read to state. `found` says what an archive holds. A citation's
stated year is not silently replaced by a retrieved opinion's year merely
because they disagree.

**A node is named for what it read.** `Reads.DOCUMENT` evidence may update
filing fields; `Reads.RECORD` evidence may settle `found`. The stage name says
where the reading happened, not what it is allowed to change. See
`docs/Document.md` for the operation and checkpoint contracts.
