...
def test_repeated_page_quote_is_grounded_at_its_own_site() -> None:
    window = "360 " + ("x" * (REACH + 2)) + " locator, 360 (court)"
    locator = window.index("locator")
    found = _find_near(
        window,
        "360",
        offset=0,
        site=Span(locator, locator + len("locator")),
    )

    assert found is not None
    assert found.start() == window.rindex("360")
