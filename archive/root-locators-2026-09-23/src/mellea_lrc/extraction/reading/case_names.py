...
from mellea_lrc.model.citations import CitationField, DocketCitation, FullCaseCitation
...
_LEAD = re.compile(
    ...
    r"compare|under|and|of|in|id\s*\.)(?:\s*,\s*|\s+)(?!\s*re\b)",
    re.I,
)
...
# Text conversion can detach a final abbreviation period (``Inc .``). Keep
# that horizontal gap inside the name, without letting it consume a newline.
_FIELD_NAME_TOKEN = r"[A-Z][\w.'’&-]*(?:[^\S\r\n]*['’][^\S\r\n]*\w[\w.'’&-]*)*(?:[^\S\r\n]+\.)?"
_FIELD_NAME_INNER = r"(?:of|the|for|in|on|at|to|by|with|and|ex|rel\.|de|van|von|del|la|le|&)"
_FIELD_NAME_PARTY = (
    rf"{_FIELD_NAME_TOKEN}(?:,?\s+(?:{_FIELD_NAME_INNER}\s+){{0,2}}{_FIELD_NAME_TOKEN}){{0,9}}"
)
...
def reread_case_names(
    ...
    for item in citations:
        if (
            item.withdrawn
            or not isinstance(item.fields, (FullCaseCitation, DocketCitation))
            or item.fields.case_name is not None
        ):
            rebuilt.append(item)
            continue
        ...
        update_field(
            revised,
            Node(
                node_id=f"case_name_read:{item.citation_id}",
                reads=Reads.DOCUMENT,
                stage="case_name_read",
                made_by=__name__,
                outcome="found",
                details={"span": {"start": name_span.start, "end": name_span.end}},
            ),
            CitationField.CASE_NAME,
            name,
            reason="Read the case name before this locator site.",
        )
...
def _explicit_name_before(text: str, before: int, floor: int) -> Span | None:
    ...
    matches = [
        ...
        # A locator prefix between the parties is a structural break, not a
        # token in the name. This matters when converted text interleaves a
        # caption or another citation with the case name.
        and not IDENTIFIER.search(match.group("name"))
    ]
    if not matches:
        return None
    match = max(matches, key=lambda found: found.start("name"))
    start, end = _strip_lead(text, *match.span("name"))
