...
therefore a docket search without a court constraint. Search, unambiguous
resolution, and bounded ambiguity review are
...
_MADE_BY = "mellea_lrc.validation.root_identity.docket"
...
async def search_docket_roots(
    ...
    *,
    ...
    The query is the stated docket number without a court constraint, so a
    wrong extracted court cannot hide a docket candidate. Candidate lists below the review
    ...
    return document.evolve(passes=(*document.passes, DOCKET_ROOT_SEARCH_STAGE))
...
async def validate_unique_docket_root_identities(document: Document) -> Document:
    ...
    return document.evolve(passes=(*document.passes, DOCKET_ROOT_UNIQUE_IDENTITY_STAGE))
...
async def resolve_docket_root_ambiguities(document: Document) -> Document:
    ...
    return document.evolve(passes=(*document.passes, DOCKET_ROOT_AMBIGUITY_RESOLUTION_STAGE))
...
async def lookup_govinfo_docket_roots(
    ...
    *,
    ...
    return document.evolve(passes=(*document.passes, GOVINFO_DOCKET_ROOT_SEARCH_STAGE))
...
async def validate_unique_govinfo_docket_root_identities(document: Document) -> Document:
    ...
    return document.evolve(passes=(*document.passes, GOVINFO_DOCKET_ROOT_UNIQUE_IDENTITY_STAGE))
...
async def resolve_govinfo_docket_root_ambiguities(document: Document) -> Document:
    ...
    return document.evolve(passes=(*document.passes, GOVINFO_DOCKET_ROOT_AMBIGUITY_RESOLUTION_STAGE))
    ...
    *,
    ...
    for record in _docket_roots(document):
        ...
        if review.status is ValidationNodeStatus.SUCCEEDED:
            mark_extraction_reviewed(record, trace_node)
        apply_docket_source_review(record, document, review, trace_node)
        judge_citation(
            record,
            trace_node,
    ...
    reviewed = document.evolve(passes=(*document.passes, DOCKET_ROOT_EXTRACTION_REVIEW_STAGE))
...
async def _requeue_reviewed_docket_roots(
    ...
    *,
    ...
    return document.evolve(passes=(*document.passes, DOCKET_ROOT_REQUEUED_SEARCH_STAGE))
...
async def validate_unique_requeued_docket_root_identities(document: Document) -> Document:
    ...
    return document.evolve(passes=(*document.passes, DOCKET_ROOT_REQUEUED_UNIQUE_IDENTITY_STAGE))
...
async def resolve_requeued_docket_root_ambiguities(document: Document) -> Document:
    ...
    return document.evolve(passes=(*document.passes, DOCKET_ROOT_REQUEUED_AMBIGUITY_RESOLUTION_STAGE))
...
async def shortlist_docket_root_metadata_candidates(document: Document) -> Document:
    ...
    filters only its returned docket metadata: ``docketNumber`` must pass 40% whitespace-relaxed edit
    ...
    for record in _docket_roots(document):
        ...
        observe_citation(record, _trace_node(shortlist, stage=DOCKET_ROOT_METADATA_SHORTLIST_STAGE))
    return document.evolve(passes=(*document.passes, DOCKET_ROOT_METADATA_SHORTLIST_STAGE))
...
async def resolve_docket_root_semantics(
    ...
    *,
    ...
    return document.evolve(passes=(*document.passes, DOCKET_ROOT_SEMANTIC_RESOLUTION_STAGE))
    ...
    *,
    ...
    if shortlist.outcome is DocketMetadataShortlistOutcome.NO_CANDIDATES:
        ...
        resolution = _future_implementation_resolution(
            ...
            reason=(
                "No returned docket metadata candidate passed the 40% docket-similarity shortlist; "
...
async def _semantic_docket_candidates(
    ...
    *,
    ...
    shortlist = build_docket_metadata_shortlist(
        ...
        complete_result_set=search.candidate_count == len(search.candidates),
        search_node_id=search.node_id,
        depends_on=(search.node_id,),
        node_id=f"{search.node_id}:semantic:shortlist",
    )
    validation = validation.append(shortlist)
    shortlisted_indices = {item.candidate_index for item in shortlist.candidates}
    eligible: list[int] = []
    unavailable = False
    for index, result in enumerate(search.candidates, start=1):
        ...
        docket_matches = docket.outcome is FieldCheckOutcome.MATCH
        if index in shortlisted_indices and docket.outcome is FieldCheckOutcome.MISMATCH:
        ...
        if index in shortlisted_indices and docket_matches:
...
def _semantic_docket_candidate_assessment(
    ...
    *,
    ...
    year_outcome: FieldCheckOutcome | None,
    ...
    year = AggregatedFieldOutcome(year_outcome.value) if year_outcome is not None else None
...
def _docket_metadata_shortlist(
    ...
    *,
    ...
    return build_docket_metadata_shortlist(
        record,
        candidates=search.candidates,
        candidate_count=search.candidate_count,
...
def build_docket_metadata_shortlist(
    record: CitationRecord,
    *,
    ...
    """Apply the common fuzzy-docket boundary to metadata records.

    Both direct docket lookup and later provider metadata discovery yield docket
    records.  They therefore share this selection rule: retain the raw result
    sets, then make candidates with at least 40% similar docket evidence
    eligible for semantic review. Court is checked independently after
    shortlisting. The filter is not a docket normalizer or an identity decision.
    """
    ...
    for index, result in enumerate(candidates, start=1):
        ...
        if not isinstance(docket_number, str) or not docket_number:
            continue
        # Capitalization is an index-presentation detail for a docket token.
        # Case-fold it only for this deliberately permissive *retrieval*
        # shortlist.  The untouched strings remain in the candidate evidence,
        # and neither this filter nor its edit score can admit an identity.
        metadata.append(
            EvidenceCandidate(
                text=docket_number.casefold(),
                value=(index, docket_number, result.get("caseName"), _candidate_parties(result)),
            )
        )
    ...
    shortlisted_candidates = tuple(
        DocketMetadataShortlistCandidate(
            candidate_index=match.candidate.value[0],
            case_name=match.candidate.value[2] if isinstance(match.candidate.value[2], str) else None,
            parties=match.candidate.value[3],
            docket_number=match.candidate.value[1],
    ...
    return DocketMetadataShortlistNode(
        ...
        outcome_message=(
            f"Shortlisted {len(shortlisted_candidates)} returned candidate(s) using 40% docket similarity."
            if shortlisted_candidates
            else "No returned candidate passed the 40% docket-similarity shortlist."
        ),
    )


def _candidate_parties(result: Mapping[str, object]) -> tuple[str, ...]:
    """Return the provider's separately indexed party names, when present.

    ``caseName`` remains the primary caption string for deterministic checks.
    The party list is retained for bounded model review because it preserves
    the individual names when a caption's formatting or ordering is unusual.
    """
    value = result.get("party")
    if not isinstance(value, (list, tuple)):
        return ()
    return tuple(party.strip() for party in value if isinstance(party, str) and party.strip())
...
def _docket_query(citation: DocketCitation) -> str:
    """Search the stated docket without filtering on another identity field."""
    assert citation.docket_number is not None
    return citation.docket_number
