...
out, and it runs out in three different ways that are one question to a reader --
...
**Not here: a page on a citation nobody read.** `Planned Parenthood Minn.,
N.D., S.D. at 732` is a case name and a page and no citation at all, because
eyecite will not search for a party name ending in a period. Finding it means
searching the document for a name, and a name is what this stage is worst at
until the roots have been resolved -- the names in the record are whatever
eyecite's parser made of them, so a search for one is a search for a guess.

That search belongs after validation has resolved the roots and the record holds
the authorities' real names, and it comes back to extraction then. Until it does,
this generator says nothing about a citation nobody read, and the three shapes
below are all about a citation that is already in the record.
...
Every candidate names the citation it is about in `about`: each one is a
correction to a record that already exists.

**Not here: a page on a citation nobody read.** `Planned Parenthood Minn.,
N.D., S.D. at 732` is a case name and a page and no citation at all, because
eyecite will not search for a party name ending in a period. Finding it means
searching the document for a name -- and a name is what this stage is worst at
until the roots have been resolved, because the names in the record are whatever
eyecite's parser made of them. A search for one is a search for a guess, and on
127 documents outside the annotated corpus that search proposed thirty sites and
not one of them was a page claim.

That search belongs after validation has resolved the roots, when the record
holds each authority's real name, and it comes back to extraction then. Until it
does, this generator says nothing about a citation nobody read.
"""
...
def _first_page(record: CitationRecord) -> int | None:
    page = getattr(record.fields, "page", None)
...
def pin_cite_sites(document: Document) -> Iterator[Candidate]:
    ...
    for record in document.citations:
        if citation_kind(record.fields) not in _CLAIMS_A_PAGE:
            continue
