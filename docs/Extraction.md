...
`Document.from_source(Path(...))` preprocesses a file; `Document.from_source("text")` treats a string as the document's text. To choose preprocessing rules, pass the result of `preprocess` to `extract`:

```python
from mellea_lrc.api import extract, preprocess

preprocessed = preprocess(Path("filing.pdf"), rules=[])
document = extract(preprocessed)
```

For the default rules, `extract(source)` preprocesses and runs the full first pass. Both `extract` and `grow_roots` are synchronous.
