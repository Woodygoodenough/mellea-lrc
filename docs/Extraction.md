...
Every public module directly under `extraction/` now writes a named Document stage. Case names, courts, dates, pin cites, colocations, roots, and docket site hunting each have their own module and stage marker. Shared colocation-aware window boundaries and site-hunting candidates/reviewers are private helpers; they never update a Document. `config/extraction.py` holds `ExtractionRules` and `stable()`, while `workflows/grow_roots.py` composes the stages. The public `mellea_lrc.api` exposes both levels without making the workflow look like a stage.
