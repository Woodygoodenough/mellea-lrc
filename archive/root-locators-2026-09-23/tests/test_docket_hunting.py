...
def test_site_admission_creates_a_minimal_docket_locator_before_field_reading() -> None:
    ...
    assert isinstance(record.fields, DocketCitation)
    assert record.fields.docket_number == "21-11854"
    # The review itself carries no surrounding fields. They remain a separate,
    # checkpointable sequence after locator admission.
    assert record.fields.court is None
    assert record.fields.date is None
    assert record.fields.case_name is None
    ...
    assert any(node.stage == "docket_locator_site_hunting" for node in record.trace)
    assert record.created_by == record.trace[0].node_id
    ...
    assert record.fields.court == "nysb"
    assert record.fields.court_text == "Bankr.  S.D.N.Y."
    assert record.fields.date is not None
    assert record.fields.date.year == "2021"
    assert record.fields.case_name is not None
    assert record.fields.case_name.text == "Ascentra v. Example"


def test_retry_after_a_decline_keeps_both_review_traces() -> None:
    document = grow_roots(preprocess("See No. 21-11854 (Bankr. S.D.N.Y. 2021)."), rules=stable())
    (site,) = suspected_dockets(document)
    first = _accepted_review(site.locator_text, site.docket_number)
    declined = SiteReview(answer=None, reason="Initial review was inconclusive.", run=first.run)

    after_decline = apply_docket_site_review(document, site, declined)
    after_retry = apply_docket_site_review(after_decline, site, first)

    assert after_decline.nodes[0].node_id == f"docket_site:{site.locator_span.start}-{site.locator_span.end}"
    assert any(
        node.node_id == f"docket_site:{site.locator_span.start}-{site.locator_span.end}:2"
        for node in after_retry.citations[0].trace
    )
    assert after_retry.nodes == after_decline.nodes
