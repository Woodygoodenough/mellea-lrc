...
class FakeClient:
    def __init__(self, clusters: list[dict[str, object]], dockets: dict[str, CourtListenerDocket | None] | None = None):
...
def exact(client: FakeClient) -> Document:
    ...
    return reporter_root_exact_lookup(roots, client=client)
...
def test_unique_passing_candidate_is_admitted_and_all_comparisons_are_saved() -> None:
    ...
    after = reporter_root_exact_ambiguity(before, client=client)

    assert client.lookup_calls == 1
    assert client.docket_calls == []
    assert after.stage_runs[-1] == STAGE
    assert after.get_stage("reporter_root_exact_lookup") == before
    root = after.roots[0]
    ...
    assert [(item.candidate_index, item.result) for item in root.case_name_judgments] == [
        (0, MatchResult.MISMATCH), (1, MatchResult.MATCH)
    ]
    ...
    assert root.identity_judgments[-1].next_step is None
    records = stage_product(after, STAGE).records
    assert {item.name for item in records} == {
        "case_name_judgments", "court_judgments", "date_judgments",
        "reporter_exact_ambiguity_resolution", "identity_judgments",
    }
    restored = Document.model_validate_json(after.model_dump_json())
    assert restored == after
    assert restored.get_stage("reporter_root_exact_lookup") == before
    with pytest.raises(ValueError, match="already completed"):
        reporter_root_exact_ambiguity(after, client=client)
...
def test_zero_or_multiple_passing_candidates_wait_for_model_review(
    ...
    after = reporter_root_exact_ambiguity(exact(client), client=client)
    root = after.roots[0]
    resolution = root.reporter_exact_ambiguity_resolution
    assert resolution is not None
    assert resolution.outcome is ReporterExactAmbiguityOutcome.REVIEW_REQUIRED
...
def test_large_candidate_set_is_preserved_without_review_or_truncation() -> None:
    ...
    resolution = root.reporter_exact_ambiguity_resolution
    assert resolution is not None
    assert resolution.outcome is ReporterExactAmbiguityOutcome.TOO_MANY_CANDIDATES
    ...
    assert root.identity_judgments[-1].next_step is IdentityNextStep.FUTURE_IMPLEMENTATION
    assert client.docket_calls == []
...
def test_candidate_docket_court_is_saved_and_checked_independently() -> None:
    ...
    assert [(item.candidate_index, item.result) for item in root.court_judgments] == [
        (0, MatchResult.MATCH), (1, MatchResult.MISMATCH)
    ]
...
def test_nonambiguous_root_does_not_gain_a_node() -> None:
    ...
    before = reporter_root_exact_lookup(roots, client=client)
    after = reporter_root_exact_ambiguity(before, client=client)
