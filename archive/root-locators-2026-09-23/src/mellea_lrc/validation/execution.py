...
from mellea_lrc.validation.aggregation import (
    ...
    run_unique_exact_locator_identity_resolution,
)
from mellea_lrc.validation.aggregation.locator_found import (
    locator_candidate_contains_reporter_locator,
)
...
from mellea_lrc.validation.candidates.selection import (
...
class CitationValidationRunner:
    ...
    def run_exact_full_reporter_locator_lookup(
        ...
        """
        stated = validation.citation.fields
    ...
    async def run_locator_found_identity(
        ...
        *,
        ...
        if _requires_opinion_court_deferral(validation, summary, candidate):
            # The linked docket can be filed in a different tribunal from the
            # cited opinion. Its court alone cannot prove a wrong citation.
            return validation.append(
                run_search_deferred_locator_identity_resolution(
                    validation,
                    depends_on=(summary.node_id,),
                    reason=(
                        "The exact reporter locator and case name agree, but only the linked docket "
                        "supplies a conflicting court; seek opinion-level court evidence."
                    ),
                )
            )
        return validation.append(run_unique_exact_locator_identity_resolution(validation, summary=summary))
    ...
    async def run_locator_candidate_validation(
        ...
        *,
        ...
        Graph:
            ...
            ├── year check
            ├── opinion court, or docket court retrieval -> court check
        ...
        if candidate.court_id is not None:
            # A court on the cited opinion cluster is more specific than the
            # linked docket's tribunal and avoids an unnecessary provider call.
            court_check_node = run_court_check(validation, evidence=candidate)
        else:
            ...
            validation = validation.append(docket_court_retrieval_node)
            court_check_node = run_court_check(validation, evidence=docket_court_retrieval_node)
        validation = validation.append(court_check_node)
    ...
    async def run_locator_candidate_case_name_recovery(
        ...
        *,
        ...
        """Reread the local name and compare it with the record in one call."""
        ...
        if (
            ...
            return validation, state
        review = await run_mellea_case_name_review(
            validation,
            trigger=exact_case_name_check,
            locator_lookup=lookup,
            candidate=candidate,
            document_text=document_text,
            session=session,
        )
        validation = validation.append(review)
        state = state.with_reextraction(review)
        state = state.with_case_name_result(
            outcome=_aggregated_mellea_outcome(review.outcome),
            evidence="mellea_combined_review",
            dependency_id=review.node_id,
        )
        return validation, state
...
async def _resolve_locator_identity_from_summary(
    ...
    *,
    ...
    """Choose among multiple exact-lookup records; never revisit a unique lookup."""
    if len(summary.candidates) < 2:
        raise ValueError("Candidate choice is reserved for ambiguous exact lookups")
...
def _requires_opinion_court_deferral(
    validation: CitationValidation,
    summary: LocatorCitationSummaryNode,
    candidate: CandidateEvaluationNode,
) -> bool:
    """Do not reject an exact opinion using only its linked docket's court."""
    if len(summary.candidates) != 1 or candidate.court_id is not None:
        return False
    assessed = summary.candidates[0]
    return (
        locator_candidate_contains_reporter_locator(validation, candidate)
        and assessed.case_name_outcome is AggregatedFieldOutcome.MATCH
        and assessed.year_outcome is not AggregatedFieldOutcome.MISMATCH
        and assessed.court_outcome is AggregatedFieldOutcome.MISMATCH
    )
