...
from mellea_lrc.model.citations import DocketCitation, DocketEntry, FullCaseCitation
...
def test_the_spans_point_at_the_docket_and_at_the_whole_citation() -> None:
    ...
    """
    (item,) = [c for c in _extract(INDICTMENT).citations if isinstance(c.fields, DocketCitation)]
...
def test_a_written_document_entry_refines_the_case_docket() -> None:
    ...
    (record,) = [item for item in _extract(text).citations if isinstance(item.fields, DocketCitation)]

    assert record.fields.docket_number == "1:25-cv-00312-RPK"
    assert record.fields.docket_entry == DocketEntry(
        number="75",
...
def test_a_docket_and_a_parallel_reporter_locator_are_two_citations() -> None:
    ...
    kinds = {type(c.fields).__name__ for c in _extract(text).citations}
...
def test_a_cmecf_docket_before_a_database_locator_is_read_once() -> None:
    ...
    dockets = [item for item in _extract(text).citations if isinstance(item.fields, DocketCitation)]

    assert [item.fields.docket_number for item in dockets] == ["1:24-cv-00123"]
...
def test_an_id_chain_attributes_to_the_docket_it_heads() -> None:
    ...
    assert isinstance(authority.root.fields, DocketCitation)
...
def test_two_courts_sharing_a_docket_number_are_two_authorities() -> None:
    ...
    assert {a.root.fields.court for a in tree.roots} == {"ncmd", "nvd"}
...
def test_a_docket_is_a_full_citation_and_a_reporter_locator_is_still_its_own() -> None:
    ...
    assert isinstance(full.fields, DocketCitation)
    assert not isinstance(full.fields, FullCaseCitation)
...
def test_the_bankruptcy_shape_needs_the_signal_in_front_of_it() -> None:
    ...
    assert not [c for c in _extract(text).citations if isinstance(c.fields, DocketCitation)]
...
def test_a_docket_number_with_no_office_is_still_a_locator() -> None:
    ...
    (citation,) = [c for c in _extract(text).citations if isinstance(c.fields, DocketCitation)]

    assert citation.fields.docket_number == "22-cv-1231"
    assert citation.fields.court_text == "W.D. Wash."
...
def test_reading_the_docket_keeps_it_out_of_the_case_name() -> None:
    ...
    reporter = next(c for c in _extract(text).citations if isinstance(c.fields, FullCaseCitation))

    assert reporter.fields.defendant == "Amazon.com, Inc."
...
def test_a_reporter_that_is_one_courts_reports_names_that_court() -> None:
    ...
    assert [c.fields.court for c in _extract(text).citations] == ["ncctapp", "scotus", "ny"]
...
def test_a_reporter_several_courts_publish_in_names_none() -> None:
    ...
    assert [c.fields.court for c in _extract(text).citations] == [None, None]
...
def test_the_court_the_filing_writes_wins_over_the_reporter_it_cites() -> None:
    ...
    assert [c.fields.court for c in _extract(text).citations] == ["ca4"]
