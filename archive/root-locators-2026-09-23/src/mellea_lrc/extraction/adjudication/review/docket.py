...
The reviewer has one job: decide whether an opaque span is a docket
...
Decide whether an opaque span in a legal filing is a docket locator
cited for a court case. A docket number is the court-assigned identifier for
one case or proceeding.

Rules:
- Quote exactly the candidate locator span supplied in the instruction. If
  that span includes a label, include it; if a label sits outside the span,
  do not add it. Do not add adjacent court, date, pinpoint, or other text.
- Quote the docket number portion of that candidate span exactly as written,
  omitting a label only when it is inside the span. Do not repair it.
...
INSTRUCTION = """
A docket-number-shaped candidate span was found in this filing window:
{{locator}}

Decide whether that exact span is a cited case docket. Set locator to the
candidate span alone, never an expanded citation or surrounding text.
