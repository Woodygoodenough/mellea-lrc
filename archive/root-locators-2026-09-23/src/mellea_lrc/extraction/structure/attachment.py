...
def _introduced(candidates: Sequence[CitationRecord]):
    ...
    """
    if len({_identifier(root.fields) for root in candidates}) != 1:
        return None
...
def _by_locator(leaf: CanonicalCitation, roots: Sequence[CitationRecord]) -> list[CitationRecord]:
    ...
    return [
        root
        for root in roots
        if getattr(root.fields, "volume", None) == volume and _reporter(root.fields) == reporter
    ]
...
def _holding_the_page(leaf: CanonicalCitation, candidates: Sequence[CitationRecord]):
    ...
    reachable = [
        ...
        if (start := _page(root.fields)) is not None and start <= page <= start + MAX_PAGES
    ]
    if not reachable:
        return None
    return max(reachable, key=lambda root: _page(root.fields) or 0)
...
def _by_name(leaf: CanonicalCitation, roots: Sequence[CitationRecord]) -> list[CitationRecord]:
    ...
    if not wanted:
        return []
    whole = [root for root in roots if wanted <= _names(root.fields)]
    return whole or [root for root in roots if wanted & _names(root.fields)]
...
def root_for(
    ...
    *,
    ...
    if kind is CitationKind.ID:
        ...
        page, start = _claimed(leaf), _page(root.fields) if root else None
