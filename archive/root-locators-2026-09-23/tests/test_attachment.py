"""Which root a leaf points at, decided from its current fields.
...
from mellea_lrc.model.operations import update_fields
...
def _page_of(roots, citation_id: str) -> str:
    return next(r.fields.page for r in roots if r.citation_id == citation_id)
...
def test_a_short_form_naming_no_root_in_its_volume_is_refused() -> None:
    roots, leaves = _read("Ashcroft v. Iqbal , 556 U.S. 662 (2009). See 410 U.S. at 116.")
    assert [root_for(leaf, roots) for leaf in leaves] == [None]


def test_id_takes_the_root_of_the_citation_before_it() -> None:
    roots, leaves = _read("Bell Atl. Corp. v. Twombly , 550 U.S. 544 (2007). Id. at 570.")
    attached = []
    for leaf in leaves:
...
def test_id_claiming_a_page_its_antecedent_cannot_hold_is_refused() -> None:
    ...
    roots, leaves = _read("Bell Atl. Corp. v. Twombly , 550 U.S. 544 (2007). Id. at 2011.")
...
def test_supra_whose_name_matches_no_root_is_refused() -> None:
    roots, leaves = _read("Ashcroft v. Iqbal , 556 U.S. 662 (2009). See Matsushita , supra , at 587.")
    assert [root_for(leaf, roots) for leaf in leaves] == [None]


def test_attachment_uses_the_current_fields_after_a_name_update() -> None:
    """A removed case name cannot attract a later supra reference."""
    roots, leaves = _read("Ashcroft v. Iqbal , 556 U.S. 662 (2009). See Iqbal , supra , at 678.")
    root = roots[0]
    assert root_for(leaves[0], roots) == root.citation_id

    update_fields(
        root,
        Node("remove-name", Reads.DOCUMENT, "review", "test", "revised"),
        {
            CitationField.CASE_NAME: None,
            CitationField.PLAINTIFF: None,
            CitationField.DEFENDANT: None,
        },
        reason="review found no case name",
    )
    assert root.fields.case_name is None
    assert any(
        item.field is CitationField.CASE_NAME and item.before is not None for item in root.field_updates
    )
