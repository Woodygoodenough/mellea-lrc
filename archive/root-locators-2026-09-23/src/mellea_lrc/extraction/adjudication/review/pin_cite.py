...
citation it is about, so the finding is a `FieldUpdate` to that citation's
...
resolved the roots. The independent pin-cite site stage applies each answer
after leaf growth; see :mod:`mellea_lrc.extraction.adjudication.pin_cite_site_validation`.
"""
...
def _matches(window: str, quote: str) -> tuple[re.Match[str], ...]:
    """Every position of a quote, with any run of whitespace matching any.
    ...
    return tuple(pattern.finditer(window))


def _find(window: str, quote: str) -> re.Match[str] | None:
    """Whether a quoted page occurs in the review window at all."""
    return next(iter(_matches(window, quote)), None)


def _find_near(window: str, quote: str, *, offset: int, site: Span) -> re.Match[str] | None:
    """Ground a repeated quote at the occurrence nearest its own citation."""
    nearby = (
        match
        for match in _matches(window, quote)
        if site.start - REACH <= offset + match.start() <= site.end + REACH
    )
    return min(nearby, key=lambda match: abs(offset + match.start() - site.end), default=None)
...
async def adjudicate_pin_cite(
    ...
    *,
    ...
    found = _find_near(window, proposed.pin_cite, offset=site.window.start, site=site.span)
    if found is None:
