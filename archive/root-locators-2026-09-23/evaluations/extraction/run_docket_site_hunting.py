...
from mellea_lrc.model.record import WITHDRAWN_HEAD_ID
...
def _docket_spans(document: Document) -> set[tuple[int, int]]:
    ...
    return {
        ...
        if isinstance(record.fields, DocketCitation) and not record.withdrawn
    }
...
def _serialized_docket_spans(document: dict[str, Any]) -> set[tuple[int, int]]:
    ...
    for record in document["citations"]:
        ...
        if citation["kind"] == "DocketCitation" and record["root_id"] != WITHDRAWN_HEAD_ID:
...
def _accepted_site_nodes(document: dict[str, Any]) -> dict[tuple[int, int], str]:
    ...
    for record in document["citations"]:
        citation = record["fields"]
        if citation["kind"] != "DocketCitation" or record["root_id"] == WITHDRAWN_HEAD_ID:
            continue
...
def _read_saved(path: Path) -> dict[str, Any] | None:
    ...
    Document.model_validate(value["final"])
    return value
...
def _occurrences(
    *,
    ...
    review_queue = [
        row for row in rows if row["outcome"] in {"unannotated_deterministic", "unannotated_site_admitted"}
    ]
    metrics = {
        ...
        "counts": {
            ...
            "locator_span_mismatches": sum(row["outcome"] == "gold_locator_span_mismatch" for row in rows),
...
async def run(
    *,
    ...
    if documents is not None:
        ...
        if unknown:
            raise ValueError(f"{dataset}: unknown documents {sorted(unknown)!r}")
        paths = [path for path in paths if path.name in selected]
        gold = {item for item in gold if item[0] in selected}
        noncase = {item for item in noncase if item[0] in selected}
        headers = {name: header for name, header in headers.items() if name in selected}
...
def main() -> None:
    ...
    parser.add_argument(
        "--no-resume", action="store_true", help="Ignore checkpoints and rerun every document."
    )
    parser.add_argument(
        "--document",
        action="append",
        dest="documents",
        help="Run only this exact filing filename; repeat for more than one.",
    )
    ...
    manifest = asyncio.run(
        run(
            ...
            documents=tuple(arguments.documents) if arguments.documents else None,
        )
    )
