...
from mellea_lrc.model.extraction_metadata import ExtractionMetadata, Relaxation
...
def _reporter(citation: CitationBase) -> Reporter | None:
    ...
    :class:`~mellea_lrc.model.citations.Reporter` -- so an ambiguous abbreviation
...
def _read_record(
    ...
    *,
    ...
    """Compose type-only creation and an initial parser field update."""
    node = Node(
        node_id=f"eyecite_initial_read:{citation_id}",
        reads=Reads.DOCUMENT,
        stage="eyecite_initial_read",
        made_by=__name__,
        outcome="identified",
        details={"kind": citation.kind.value},
    )
    record = create_citation(citation_id, citation.kind, node)
    update_fields(record, node, field_values(citation), reason="Initial eyecite reading.")
    if root_id is not None:
        record = assign_root(record, root_id, node, resolves_to=resolves_to)
    elif resolves_to is not None:
        record = assign_antecedent(record, resolves_to, node)
    return record
...
def _read(
    ...
    for eyecite_citation, citation_id in citation_ids:
        ...
        if is_leaf(canonical):
            ...
            extracted.append(
                _read_record(citation_id, canonical, resolves_to=antecedent_map.get(citation_id))
            )
    ...
    if leaves:
        ...
        leaf_records = tuple(
            _read_record(citation_id, citation, root_id=citation_id)
        ...
        leaves = [(item.citation_id, item.fields, antecedents[item.citation_id]) for item in parsed_leaves]
...
def _with_leaves(
    ...
    **Decided from `stated`**, by
    ...
    when its antecedent is in the document, so the two can be compared.
    ...
    `findings` rather than to nothing. See :mod:`mellea_lrc.model.findings`.
    ...
    roots = [record for record in document.citations if not is_leaf(record.fields) and record.is_root]
    ...
    for citation_id, canonical, antecedent in sorted(leaves, key=lambda item: item[1].span.start):
        known_ids = {record.citation_id for record in (*document.citations, *grown)}
        # Id. and Ibid. refer to the preceding citation, which can be a statute
        # or journal omitted from this case-citation document. In that case,
        # attaching to the preceding *retained case* would invent an authority.
        if (
            citation_kind(canonical) is CitationKind.ID
            and antecedent is not None
            and antecedent not in known_ids
        ):
            ungrown.append(
                Finding(
                    ...
                    message="Id. antecedent is not a retained case citation, so no leaf was built",
                    span=canonical.span,
                    citation=canonical,
                )
            )
        ...
        if root_id is None:
            ...
            continue
        # A named short form can still attach by its stated fields even when
        # eyecite linked it to an excluded citation. A dangling parser link is
        # not a valid graph edge, so only retain links to present citations.
        resolved_antecedent = antecedent if antecedent in known_ids else None
        leaf = _read_record(citation_id, canonical, root_id=root_id, resolves_to=resolved_antecedent)
        grown.append(leaf)
    ...
    # The next Document must have a valid graph immediately. A leaf attached
    # to an already-withdrawn root is moved to the virtual head before evolve.
    withdraw_leaves_of_withdrawn_roots(citations)
    grown_document = document.evolve(
...
def extract_from_plain_text(
    ...
    *,
    ...
    if source_path is not None:
        preprocessed = preprocessed.model_copy(
            update={"source_metadata": replace(preprocessed.source_metadata, path=source_path)},
        )
