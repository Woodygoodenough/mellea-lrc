...
def mark_full_reporter_locator_hunting_skipped(
    ...
    *,
    ...
    return _with_pass(document.evolve(nodes=(*document.nodes, node)), REPORTER_SITE_STAGE)
...
def resolve_colocations(
    ...
    *,
    ...
    return _with_pass(document.evolve(citations=citations), COLOCATION_STAGE)
...
def _read_locator_candidates(
    ...
    *,
    ...
    return tuple(record for record in read.citations if isinstance(record.fields, wanted))
    ...
    *,
    ...
    for candidate in candidates:
        if candidate.citation_id in held:
            continue
        citation = _locator_only(candidate.fields)
        if not isinstance(citation, kind):
            continue
        node = Node(
            node_id=f"{stage}:{candidate.citation_id}",
            reads=Reads.DOCUMENT,
            stage=stage,
            made_by=_MADE_BY,
            outcome="identified",
        ...
        record = create_citation(candidate.citation_id, citation.kind, node)
        update_fields(
            record,
            node,
            field_values(citation),
            reason="Initial locator reading.",
        )
        admitted.append(record)
        held.add(record.citation_id)

    appended = document.evolve(
...
def _with_pass(document: Document, stage: str) -> Document:
    ...
    if stage in document.passes:
        return document
    return document.evolve(passes=(*document.passes, stage))
