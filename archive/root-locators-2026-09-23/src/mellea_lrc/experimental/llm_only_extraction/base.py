...
from mellea_lrc.model.citations import placed
from mellea_lrc.model.document import Document
from mellea_lrc.model.extraction_metadata import ExtractionBackend, ExtractionMetadata
...
class MelleaExtractorBase(abc.ABC):
    ...
    def _to_extracted_citation(self, found: FoundCitation) -> CitationRecord:
        ...
        node = Node(
            node_id=f"llm_locator_read:{citation_id}",
            reads=Reads.DOCUMENT,
            stage="llm_locator_read",
            made_by=__name__,
            outcome="identified",
        )
        record = create_citation(citation_id, CitationKind.UNKNOWN, node)
        update_fields(
            record,
            node,
            field_values(
            ...
            reason="Initial model locator reading.",
        )
        return record
