...
from mellea_lrc.model.citations import FullCaseCitation, IdCitation, ShortCaseCitation
...
def _locators(text: str, relaxation: Relaxation = Relaxation.BOUNDED) -> set[str]:
    return {
        f"{c.fields.volume} {c.fields.reporter} {c.fields.page}"
        for c in _extract(text, relaxation).citations
        if isinstance(c.fields, FullCaseCitation)
    }


def _only_full_case(text: str, relaxation: Relaxation = Relaxation.BOUNDED):
    (citation,) = [
        item for item in _extract(text, relaxation).citations if isinstance(item.fields, FullCaseCitation)
    ]
    return citation
...
def test_a_short_form_is_read_through_doubled_spaces(relaxation: Relaxation) -> None:
    ...
    short = [c for c in citations if isinstance(c.fields, ShortCaseCitation)]

    assert len(short) == 1
    assert short[0].fields.volume == "367"
    assert short[0].fields.reporter.canonical == "P.3d"
...
def _ids(text: str, relaxation: Relaxation) -> list[str]:
    ...
    return [c.matched_text for c in citations if isinstance(c.fields, IdCitation)]
...
def test_the_same_text_reads_the_same_way_in_every_process(relaxation: Relaxation) -> None:
    ...
    pacific = [c for c in citations if isinstance(c.fields, FullCaseCitation) and c.fields.volume == "206"]

    assert len(pacific) == 1
    assert pacific[0].fields.reporter.as_written == "P."
    assert pacific[0].fields.page == "327"
