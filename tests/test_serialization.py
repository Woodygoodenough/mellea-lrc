...
def test_validated_document_round_trip_supports_every_current_node_type() -> None:
    ...
    nodes = (
        ...
        QuotationCheckNode(
            node_id=f"{candidate_id}:reporter_page_retrieval:quotation_check",
            status=ValidationNodeStatus.SUCCEEDED,
            outcome=QuotationCheckOutcome.ALTERED,
            context_span=Span(0, 100),
            passages=(
                QuotedPassageEvidence(
                    quoted_text="segregated public schools are not identical",
                    quoted_span=Span(20, 62),
                    outcome="altered",
                    score=0.92,
                    page_span=Span(20, 60),
                    page_text="segregated public schools are not equal",
                    substitutions=(("identical", "equal"),),
                ),
            ),
            depends_on=(f"{candidate_id}:reporter_page_retrieval",),
        ),
