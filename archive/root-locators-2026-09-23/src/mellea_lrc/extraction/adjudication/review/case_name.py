...
``names_a_citation``
    ...
    citation and nothing about it says the citation is wrong. What the finding
    carries is **where the name is written**, so a consumer can hold a fuller
    name when the original citation parse found only an abbreviated party.
...
What a reader decides here is not written back before the next site is read,
and the roots list is built once. A root that was read with no case name stays
nameless for every site in the pass, so if the review of the name written near
...
no distance limit or ordering requirement: what decides `uncited_case` is
whether the document cites the case anywhere, because a reader can reach it
from any section.
"""
...
"not_a_citation" whatever else is true. If it is, go to the second question.
    ...
    Watch for a case name used only as the LABEL OF A RULE. When a surname
    modifies a noun such as "standard", "framework", or "test", and the
    sentence relies on the doctrine rather than that decision, answer
    "not_a_citation". When the sentence instead attributes a holding or other
    act to the decision itself, go on to the second question.
...
"short_form" - it does, but not here: the case is in the roots list and this
    ...
    whether the mention sits in a citation's place or inside the grammar of a
    sentence. In either position, a reference to the same case without a
    locator is a short form of a root already in the document.
...
- plaintiff and defendant, for "names_a_citation" and "short_form": the name
            ...
            Preserve each party's written name. A case styled "In re" or
            "Ex parte" has no adverse plaintiff: set plaintiff=null and put
            the named subject in defendant. Leave both null for the other two
            readings.
- reason    one sentence
...
- Stop at the end of the name. A docket number, reporter citation, or year
  after it is a separate identifier, not part of the name.
...
- Damage belongs in "name" and must be repaired in "plaintiff" and
  "defendant". Do not otherwise change the words: the parties are checked
  against the quote and an unsupported party will not pass.
...
class AdjudicatedCaseName:
    ...
    This follows eyecite's convention: an "In re" name and a single-party
    short form leave `plaintiff` unset. A patch that filled `plaintiff` instead
    would disagree with citations parsed by the rules.
    """
    reason: str = ""
    match_method: str = ""
...
def _citation_line(
    ...
    known = record.fields.case_name if record is not None else citation.case_name
    parse = record.fields if record is not None else citation.fields
    parties = (
...
# neither party, following eyecite's reading of single-party names.
    ...
    *,
    ...
    if proposed.reading is Reading.NAMES_A_CITATION:
        ...
        chosen_name = " ".join(
            part
            for part in (
                getattr(chosen.fields, "plaintiff", None),
                getattr(chosen.fields, "defendant", None),
                getattr(chosen.fields, "antecedent", None),
            )
            if part
        )
