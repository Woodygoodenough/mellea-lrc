...
def test_metadata_shortlist_filters_a_large_returned_page_before_one_model_choice(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A partial response may yield a positive, traced semantic docket selection."""
    document = _document(date=None)
    document.citations[0].stated = replace(document.citations[0].stated, docket_number="20 Civ. 6835")
    client = _DocketSearchClient(
        count=21,
        results=[
    ...
    async def choose_first(validation, *, summary, shortlist, **kwargs):
        ...
        assert [candidate.candidate_index for candidate in summary.candidates] == [1, 2]
        return MelleaLocatorCandidateChoiceNode(
    ...
    assert [candidate["candidate_index"] for candidate in payload["candidates"]] == [1, 2]
    assert root.judgement(Question.IDENTITY).outcome == "resolved"
