...
def test_a_site_needing_only_a_re_read_costs_no_call() -> None:
    ...
    assert citation.fields.volume == "33"
    assert citation.fields.reporter.canonical == "F.4th"
    assert citation.fields.page == "693"
    assert citation.fields.date.year == "2022"
    assert citation.fields.court == "ca2"
...
def test_promoting_a_reviewed_locator_repairs_it_and_parses_the_result() -> None:
    ...
    assert citation is not None
    assert isinstance(citation.fields, FullCaseCitation)
    assert citation.fields.page == "662"
    assert citation.fields.date.year == "2009"
