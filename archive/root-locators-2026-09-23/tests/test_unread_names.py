...
def test_a_reviewed_name_outside_the_original_citation_span_is_masked() -> None:
    text = "Bell Atl. Corp. v. Twombly; 550 U.S. 544. Roe v. Wade was also cited."
    name_text = "Bell Atl. Corp. v. Twombly"
    name = Span(0, len(name_text))
    locator_start = text.index("550 U.S. 544")
    locator = Span(locator_start, locator_start + len("550 U.S. 544"))
    citation = read_citation(
        citation_id="root",
        fields=placed(
            FullCaseCitation(case_name=CaseName(span=name, text=name_text)),
            span=locator,
    ...
    unread = unread_case_names(text, (citation,))

    assert [text[span.start : span.end] for span in unread] == ["Roe v. Wade"]
