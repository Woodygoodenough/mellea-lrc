...
from mellea_lrc.validation.citation_lookup.exact import run_exact_locator_lookup
...
def test_full_reporter_locator_runs_identity_without_pinpoint_work() -> None:
    ...
    extracted = _document(
        FullCaseCitation(
            ...
            defendant="Board",
            volume="347",
            reporter=Reporter(as_written="U.S."),
            page="483",
            date=CitationDate(year="1954"),
            court="scotus",
        )
    ...
    lookup, candidate, exact_name, year, court, assessment, summary, resolution = progression.nodes
    ...
    assert court.depends_on == (candidate.node_id,)
...
def test_found_field_checks_treat_missing_year_as_a_full_match() -> None:
    """A null date judgment does not downgrade a confirmed reporter identity."""
    extracted = _document(
        FullCaseCitation(
            plaintiff="Brown",
            defendant="Board",
    ...
    _, _, exact_name, year, court, assessment, summary, resolution = (
        _validate(extracted, client).citations[0].nodes
    )
    assert exact_name.outcome is FieldCheckOutcome.MATCH
    assert year.outcome is None
    assert court.outcome is FieldCheckOutcome.MATCH
    assert assessment.outcome is LocatorCandidateAssessmentOutcome.MATCH
...
def test_exact_locator_check_rejects_a_nearby_reporter_result() -> None:
    """A found response must carry the requested volume, reporter, and page."""
    document = _document(
        FullCaseCitation(
            plaintiff="Brown",
            defendant="Board",
    ...
    candidate = run_locator_candidate_evaluation(
        ...
        cluster=CourtListenerOpinionCluster(
            case_name="Brown v. Board",
            citations=(CourtListenerOpinionClusterCitation(volume="347", reporter="U.S.", page="482"),),
        ),
    )

    assert locator_candidate_contains_reporter_locator(validation, candidate) is False


@pytest.mark.parametrize("spelling", ["Fed. Appx.", "Fed. App'x"])
def test_exact_locator_check_accepts_a_canonical_reporter_spelling(spelling: str) -> None:
    """An edition alias in the filing can match the provider's canonical citation."""
    document = _document(
        FullCaseCitation(
            plaintiff="United States",
            defendant="Rucker",
            volume="188",
            reporter=Reporter(as_written=spelling, short_name="F. App'x"),
            page="772",
            date=CitationDate(year="2006"),
            court="ca10",
        )
    )
    validation = CitationValidation(citation=document.citations[0])
    candidate = run_locator_candidate_evaluation(
        ...
        depends_on=(),
        cluster=CourtListenerOpinionCluster(
            case_name="United States v. Rucker",
            citations=(CourtListenerOpinionClusterCitation(volume="188", reporter="F. App'x", page="772"),),
        ),
    )

    assert locator_candidate_contains_reporter_locator(validation, candidate) is True
...
def test_found_field_checks_record_mismatch_without_failing_execution(
    ...
    async def fake_review(_validation: object, *, trigger: ExactCaseNameCheckNode, **_kwargs):
        return MelleaCaseNameReviewNode(
            ...
            case_name=CaseName(span=Span(0, len("Brown v. Board")), text="Brown v. Board"),
            plaintiff="Brown",
            defendant="Board",
            rationale="The parties differ.",
            depends_on=(trigger.node_id,),
        )

    monkeypatch.setattr("mellea_lrc.validation.execution.run_mellea_case_name_review", fake_review)

    nodes = _validate(extracted, client).citations[0].nodes
    exact_name = next(node for node in nodes if isinstance(node, ExactCaseNameCheckNode))
    year = next(node for node in nodes if isinstance(node, YearCheckNode))
    court = next(node for node in nodes if isinstance(node, CourtCheckNode))
    review = next(node for node in nodes if isinstance(node, MelleaCaseNameReviewNode))
    assessment = next(node for node in nodes if isinstance(node, LocatorCandidateAssessmentNode))
    summary = next(node for node in nodes if isinstance(node, LocatorCitationSummaryNode))
    resolution = next(node for node in nodes if isinstance(node, LocatorIdentityResolutionNode))
    assert exact_name.outcome is FieldCheckOutcome.MISMATCH
    assert year.outcome is FieldCheckOutcome.MISMATCH
    assert review.outcome is MelleaCaseNameCheckOutcome.MISMATCH
    ...
    assert not any(isinstance(node, MelleaLocatorCandidateChoiceNode) for node in nodes)
    assert resolution.outcome is LocatorIdentityResolutionOutcome.NO_MATCH
    assert "repars" not in resolution.outcome_message
...
def test_found_unavailable_extraction_with_retrieved_name_still_reviews_source(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A missing local extraction still gets one combined source review."""
    ...
    async def fake_review(
        ...
        *,
        ...
        assert candidate.case_name == "In re Soundview Elite, Ltd."
        ...
        return MelleaCaseNameReviewNode(
            ...
            plaintiff="Brown",
            defendant="Board",
            rationale="The filing names a different case.",
            depends_on=(trigger.node_id,),
        )

    monkeypatch.setattr(
        "mellea_lrc.validation.execution.run_mellea_case_name_review",
        fake_review,
    )
...
def test_mellea_case_name_reextraction_uses_only_local_context(
    ...
    assert node.case_name.span == Span(0, len("Brown v. Board"))
    assert node.case_name.text == "Brown v. Board"
    assert document.text[node.case_name.span.start : node.case_name.span.end] == node.case_name.text
...
def test_search_candidate_uses_semantic_check_without_reextracting(
    ...
    assert progression.nodes[2].outcome is (
        FieldCheckOutcome.MATCH if source is CandidateEvaluationSource.OPINION_SEARCH else None
    )
...
def test_ambiguous_lookup_sends_all_reviewed_candidates_to_model(
    ...
    spec = calls[0]
    assert spec.grounding_context.keys() == {"local_context"}
    assert "flag alone does not prove identity" in spec.prefix
    assert "Compare the forms semantically" in spec.prefix
    ...
    assert '"local_case_name"' in spec.user_variables["reviewed_candidates_json"]
    assert '"local_case_name_evidence"' in spec.user_variables["reviewed_candidates_json"]
    with pytest.raises(ValueError, match="at least one eligible candidate"):
        asyncio.run(
            run_mellea_locator_candidate_choice(
                progression,
                summary=summary,
...
def test_unique_exact_court_mismatch_is_programmatic_no_match(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A conflicting court on the unique record ends without candidate selection."""
    extracted = _document(
        FullCaseCitation(
            plaintiff="Brown",
            defendant="Board",
    ...
    async def forbidden_choice(*_args, **_kwargs):
        raise AssertionError("Unique exact lookup must not ask a model to choose a candidate")

    monkeypatch.setattr(
        "mellea_lrc.validation.execution.run_mellea_locator_candidate_choice", forbidden_choice
    )
    progression = _validate_identity(extracted, client).citations[0]
    resolution = progression.identity_resolution
    ...
    assert "court conflicts" in resolution.outcome_message
    assert not any(isinstance(node, MelleaLocatorCandidateChoiceNode) for node in progression.nodes)


def test_exact_lookup_treats_multiple_clusters_as_ambiguous_when_status_is_ok() -> None:
    """A provider's 200 response may still contain a bounded candidate set."""
    document = _document(FullCaseCitation(volume="347", reporter="U.S.", page="483"))
    client = LookupClient(
        CourtListenerCitationLookup(
            citation="347 U.S. 483",
            status=200,
            clusters=(
                CourtListenerOpinionCluster(cluster_id="one"),
                CourtListenerOpinionCluster(cluster_id="two"),
            ),
        )
    )

    lookup = run_exact_locator_lookup(CitationValidation(citation=document.citations[0]), client=client)

    assert lookup.outcome is LocatorLookupOutcome.AMBIGUOUS
    assert lookup.candidate_count == 2
    assert lookup.candidate_clusters is not None
    assert [candidate.cluster_id for candidate in lookup.candidate_clusters] == ["one", "two"]
