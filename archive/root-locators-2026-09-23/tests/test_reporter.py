...
from mellea_lrc.model.citations import FullCaseCitation, FullLawCitation
...
def _first(text: str, kind: type = FullCaseCitation):
    ...
    return next(item.fields for item in document.citations if isinstance(item.fields, kind))
