...
class RootBodySearchOutcome(str, Enum):
    """Results of one bounded provider body-corroboration search."""
...
class RootBodySearchSource(str, Enum):
    """Native record shape returned by one corpus body search."""

    COURTLISTENER_CLUSTER = "courtlistener_cluster"
    COURTLISTENER_DOCKET = "courtlistener_docket"
...
class CandidateEvaluationSource(str, Enum):
    ...
    FULL_REPORTER_METADATA_SEARCH = "full_reporter_metadata_search"
    GOVINFO_FULL_REPORTER_METADATA_SEARCH = "govinfo_full_reporter_metadata_search"
    OPINION_SEARCH = "opinion_search"
    RECAP_SEARCH = "recap_search"
    COURTLISTENER_CLUSTER_BODY_SEARCH = "courtlistener_cluster_body_search"
    COURTLISTENER_DOCKET_BODY_SEARCH = "courtlistener_docket_body_search"
    GOVINFO_BODY_SEARCH = "govinfo_body_search"
...
class LocatorIdentityResolutionOutcome(str, Enum):
    ...
    DEFERRED_TO_OPEN_WEB_SEARCH = "deferred_to_open_web_search"
...
class ExactLocatorLookupNode:
    ...
    retrospective_date: str | None = None
...
@dataclass(frozen=True, slots=True)
class MelleaCaseNameReviewNode:
    """One source reread and semantic name comparison for one retrieved record."""

    node_id: str
    status: ValidationNodeStatus
    outcome: MelleaCaseNameCheckOutcome
    retrieved_case_name: str | None
    case_name: CaseName | None
    plaintiff: str | None
    defendant: str | None
    rationale: str | None
    depends_on: tuple[str, ...]
...
class MelleaCaseNameReextractionNode:
    """A source-grounded case name re-extracted from citation-local text."""
...
class DocketMetadataShortlistCandidate:
    """One provider metadata result retained by an explicit local shortlist."""
...
class DocketMetadataShortlistNode:
    ...
    The raw search node remains the complete record of what the provider
    returned. This node records only candidate indexes which passed its stated
    local boundary, such as fuzzy-docket evidence or a bounded
    source-grounded caption query. It does not decide docket equivalence or
    identity.
    """
...
@dataclass(frozen=True, slots=True)
class BodySearchAttempt:
    """One CourtListener body query, including a refinement that yielded no hits."""
    ...
    used_for_candidates: bool
    error: str | None = None


@dataclass(frozen=True, slots=True)
class RootBodySearchNode:
    """One bounded corpus-body search and its provider-native candidates.

    The query is source-grounded in the locator written in the filing.  The
    result only identifies a possible citing document. Its metadata belongs to
    that document. A grounded citation in its actual body can corroborate the
    source citation in a later review stage.
    """
...
class MelleaDocketCitationReextractionNode:
    ...
    The docket number and case-name span are grounded in the filing. Other
    proposed fields retain their copied source text for typed readers. This
    node never copies information from a retrieved record.
    """
...
class LocatorCandidateAssessmentNode:
    ...
    year_outcome: AggregatedFieldOutcome | None
    extracted_court_id: str | None
    retrieved_court_id: str | None
...
class MelleaLocatorCandidateChoiceNode:
    ...
    """Whether a selected docket candidate has an equivalent written identifier."""
    case_name_equivalent: bool | None = None
    """Whether a selected candidate's caption names the same matter."""
...
@dataclass(frozen=True, slots=True)
class YearCheckNode:
    """Deterministic date comparison at the precision supported by evidence."""

    node_id: str
    status: ValidationNodeStatus
    outcome: FieldCheckOutcome | None
    extracted_year: str | None
    retrieved_year: str | None
    depends_on: tuple[str, ...]
    status_message: str | None = None
