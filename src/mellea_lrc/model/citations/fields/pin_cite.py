...
class PinCiteField(CitationField[PinCiteValue]):
    ...
    def from_source(cls, source: str, span: Span, *, node_id: str) -> Self:
        ...
        return cls(node_id=node_id, quote=quote, span=span, normalized=normalize_pin_cite(quote))

    @model_validator(mode="after")
    def _validate_normalization(self) -> Self:
        if not self.normalized or self.normalized != normalize_pin_cite(self.quote):
            raise ValueError("Pin cite normalization does not match its quote")
        return self
