...
def run_exact_case_name_check(
    ...
    *,
    ...
    ``extracted_case_name`` lets a source-grounded combined re-extraction
    supply the filing's local reading before a dedicated name reader has
    materialized a typed ``CaseName``.  It never comes from the retrieved
    candidate and therefore does not weaken the comparison boundary.
    """
    citation = validation.citation.fields
    extracted = extracted_case_name or (
