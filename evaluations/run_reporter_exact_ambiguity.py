"""Resume saved exact-lookups through the rule-only ambiguity stage.
...
Each output Document retains the complete exact-lookup checkpoint and can
...
from evaluations.run_reporter_exact_lookup import _checkpoint, _persist_document, _save, _source_document
from mellea_lrc.validation.reporter_root_exact_ambiguity import STAGE, reporter_root_exact_ambiguity
...
def run_documents(
    ...
    """Use immutable exact-lookup documents and resume safely by filing."""
    ...
    for name in names:
        ...
        for filename, metadata in sorted(manifest.items()):
            ...
            exact_path = input_run_dir / "documents" / name / f"{filename}.json"
            ...
            if output_path.exists():
                ...
                if saved.get_stage(EXACT_STAGE) != exact:
                    raise ValueError(f"{output_path}: exact lookup checkpoint differs from input")
                counts["ambiguity_reused"] += 1
                continue
            resolved = reporter_root_exact_ambiguity(exact)
