...
:class:`~mellea_lrc.model.record.CitationRecord` like every other one?
...
from mellea_lrc.model.citations import CanonicalCitation, DocketCitation, placed
from mellea_lrc.model.operations import create_citation, field_values, update_fields
...
def _promoted_record(citation_id: str, citation: CanonicalCitation) -> CitationRecord:
    """Create a typed citation and materialize the promoted parser reading."""
    node = Node(
        node_id=f"promotion_initial_read:{citation_id}",
        reads=Reads.DOCUMENT,
        stage="promotion_initial_read",
        made_by=__name__,
        outcome="identified",
        details={"kind": citation.kind.value},
    )
    record = create_citation(citation_id, citation.kind, node)
    update_fields(record, node, field_values(citation), reason="Initial promoted locator reading.")
    return record
...
def promote(text: str, candidate: Candidate) -> CitationRecord | None:
    ...
    for citation in found:
        ...
        return _promoted_record(
            citation_id_for(locator_span, citation.matched_text()),
            _placed(text, canonical, full_span, locator_span, citation.matched_text()),
        )
    return None
...
def promote_locator(text: str, locator: AdjudicatedLocator) -> CitationRecord | None:
    ...
    *   ``full_span`` is found in repaired coordinates and mapped back through
    ...
    for citation in found:
        ...
        return _promoted_record(
            citation_id_for(locator_span, locator.text),
            ...
            # lies past its end.
            _placed(text, promoted, full_span, locator_span, locator.text),
        )
    return None
...
def promote_docket_locator(
    ...
    return _promoted_record(citation_id_for(locator_span, site.locator_text), docket)
