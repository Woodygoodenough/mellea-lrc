...
def _name_count(document: Document) -> int:
    return sum(
        citation.fields.case_name is not None
        for citation in document.active_citations
        if hasattr(citation.fields, "case_name")
    )
...
def run(*, documents: Path, output: Path, start: int, limit: int, resume: bool) -> dict[str, object]:
    ...
    for index, source in enumerate(inputs, start=1):
        ...
        if resume and destination.exists():
            document = Document.model_validate(json.loads(destination.read_text(encoding="utf-8")))
        else:
            original = Document.model_validate(json.loads(source.read_text(encoding="utf-8")))
