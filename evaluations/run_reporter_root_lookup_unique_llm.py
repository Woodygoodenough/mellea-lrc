"""Resume saved single-candidate reporter lookups through model review.
...
Each output Document retains its lookup checkpoint and the complete model
attempt trace. An interrupted run can resume without repeating saved filings.
"""
...
from mellea_lrc.validation.reporter_root_lookup_unique_llm import STAGE, reporter_root_lookup_unique_llm


def _run_spec(data_root: Path, input_run_dir: Path, run_dir: Path) -> str:
    """Validate the saved lookup run and describe the new checkpoint."""
    ...
    if input_spec.get("checkpoints", [])[-1:] != [LOOKUP_STAGE]:
        raise ValueError("Input run must end at the reporter lookup checkpoint")
    return (
        json.dumps(
            {
                **input_spec,
...
async def run_documents(
    ...
    """Continue immutable lookup Documents and commit each reviewed filing."""
    ...
    for name in names:
        ...
        for filename, metadata in sorted(manifest.items()):
            ...
            if output_path.exists():
                ...
                continue
            try:
                reviewed = await reporter_root_lookup_unique_llm(lookup)
            except Exception:
                print(
                    f"Reporter model review stopped at {name}/{filename}; rerun to resume.", file=sys.stderr
                )
                raise
            _persist_document(output_path, reviewed)
            counts["review_created"] += 1
            print(f"Saved reporter model review: {name}/{filename}", flush=True)
    return counts
