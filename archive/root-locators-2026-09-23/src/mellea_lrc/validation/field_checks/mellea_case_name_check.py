...
garbled, materially incomplete, or different-case names are mismatches. A
misspelled party or title is also a mismatch, even if the intended case is
recognizable; do not treat a spelling error as an abbreviation.
""".strip()
...
def _case_name_from_reextraction(node: MelleaCaseNameReextractionNode) -> str | None:
    return node.case_name.text if node.case_name is not None else None
