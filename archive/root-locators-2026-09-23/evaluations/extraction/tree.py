...
``mellea``
    the compositional root and leaf stages, then independent model reviews of
    unread case names and uncertain pin-cite sites.
...
Rule 10.9 lets a filing write a case name and stop, and finding one requires a
separate review after leaf growth. The deterministic baseline arms do not run
that review; the model arm does. `--score-bare-names` includes this separate
...
from mellea_lrc.api import (
    grow_leaves as grow_document_leaves,
)
from mellea_lrc.api import (
    grow_roots as grow_document_roots,
)
from mellea_lrc.api import (
...
from mellea_lrc.model.citations import CitationKind, citation_kind
from mellea_lrc.model.document import Document
from mellea_lrc.model.extraction_metadata import Relaxation
from mellea_lrc.model.pin_cites import PinCiteKind
...
class Arm:
    ...
    def needs_model(self) -> bool:
        ...
        return self.hunt_leaf_names or self.validate_pin_sites
...
ARMS = {
    ...
    "augmented": Arm(Relaxation.FULL, components="+ this project's rules", attach=Attachment.EYECITE),
    "grown": Arm(Relaxation.FULL, components="+ leaves grown from `stated`"),
    "mellea": Arm(
        Relaxation.FULL,
        components="+ leaf-name and pin-cite site reviews after leaf growth",
...
_LEAF_KINDS = frozenset({"IdCitation", "ShortCaseCitation", "SupraCitation", "ReferenceCitation"})
...
def _from_rules(extracted: Document, *, dockets: bool) -> dict[tuple[int, int], dict[str, Any]]:
    ...
    for citation in extracted.citations:
        kind = citation_kind(citation.fields)
        ...
        rows[span] = _row(
            citation_kind(citation.fields).value,
            span,
            ...
            getattr(citation.fields, "court", None)
            if citation_kind(citation.fields) is CitationKind.DOCKET
            else None,
            getattr(citation.fields, "court", None),
...
def _written_date(citation: CitationRecord) -> str | None:
    ...
    """
    date = getattr(citation.fields, "date", None)
...
_MONTHS = {
    name: number
    for number, name in enumerate("jan feb mar apr may jun jul aug sep oct nov dec".split(), start=1)
}
...
async def run_document(
    ...
    **Through the artifact.** What is scored is what comes back from
    `Document.model_dump` and `Document.model_validate`, not the
    ...
    if arm.needs_model:
        if session is None:
            msg = "Model review arms require a configured session."
            raise ValueError(msg)
        # The model arm follows the public Document stages. Both optional
        # reviews see the leaves, and each produces its own checkpoint.
        rules = stable(relaxation=arm.relaxation)
        extracted = await grow_document_roots(Document.from_plain_text(text, rules=rules), rules=rules)
        if arm.needs_identity and identity_run is not None:
            extracted = settled(extracted, identity_run / f"{Path(name).stem}.json")
        extracted = await grow_document_leaves(extracted, attach=arm.attach)
        if arm.hunt_leaf_names:
            extracted = await hunt_leaf_case_names(extracted, session=session)
        if arm.validate_pin_sites:
            extracted = await validate_pin_cite_sites(extracted, session=session)
    else:
        # Baseline arms retain the isolated eyecite reader for comparison.
    ...
    return _from_rules(Document.model_validate(payload), dockets=arm.reads_dockets)
...
def _replayed(replay: Path, name: str, *, dockets: bool) -> dict[tuple[int, int], dict[str, Any]]:
    ...
    return _from_rules(Document.model_validate(payload), dockets=dockets)
    ...
    *,
    ...
    for header, body in read_documents(dataset):
        ...
        if defer_bare_names:
            ...
            annotated = [row for row in annotated if not _is_bare_name(row.get("kind"), row.get("pin_cite"))]
        ...
        for row in annotated:
            ...
            want_any_court = None if row["kind"] in _LEAF_KINDS else (row.get("court") or {}).get("id")
            if want_any_court:
