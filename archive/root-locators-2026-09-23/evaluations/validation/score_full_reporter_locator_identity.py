...
def _identity_decisions(artifacts: Path) -> dict[tuple[str, int, int], str]:
    ...
    for path in sorted(artifacts.glob("*.json")):
        ...
        for citation in payload.get("citations", []):
            ...
            if source.get("kind") != "FullCaseCitation" or citation.get("root_id") != citation.get(
                "citation_id"
            ):
                continue
...
def _resolution_outcome(citation: dict[str, Any]) -> str | None:
    """Return the current identity judgment, with a trace fallback for old runs."""
    judgements = citation.get("judgements")
    identity = judgements.get("identity") if isinstance(judgements, dict) else None
    outcome = identity.get("outcome") if isinstance(identity, dict) else None
    if isinstance(outcome, str):
        return outcome

    # Older artifacts predate citation-level current judgments. Their last
    # identity node is terminal; choosing the first node would incorrectly
    # score an earlier deferral after a later body-corroboration resolution.
    for node in reversed(citation.get("trace", [])):
