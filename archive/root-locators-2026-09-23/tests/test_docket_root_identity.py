...
import mellea_lrc.validation.root_identity.docket as docket_roots
...
def test_docket_search_and_unique_identity_write_one_resumable_root_decision() -> None:
    ...
    checkpoint = Document.model_validate(searched.model_dump(mode="json"))
    ...
    assert search_node.details["validation"]["query"] == "1:24-cv-08760"
...
def test_metadata_shortlist_ignores_docket_identifier_capitalization() -> None:
    """Capitalization may help retrieve a candidate but never itself resolves identity."""
    document = _document(date=None)
    revise_citation(document.citations[0], docket_number="21-CV-01915-PAB-KAS")

    shortlist = docket_roots.build_docket_metadata_shortlist(
        document.citations[0],
        candidates=(
            {
                "docket_id": 1,
                "docketNumber": "1:21-cv-01915",
                "caseName": "Perez v. Sunbeam Products, Inc.",
                "court_id": "nysd",
            },
        ),
        candidate_count=1,
        complete_result_set=True,
        search_node_id="test:search",
        depends_on=("test:search",),
        node_id="test:shortlist",
    )

    assert shortlist.outcome.value == "shortlisted"
    assert shortlist.candidates[0].docket_number == "1:21-cv-01915"
    assert shortlist.candidates[0].similarity_percent < 100


def test_govinfo_fallback_resolves_a_dated_citation_without_a_date_conflict() -> None:
    """A package may establish root identity while later work verifies the cited order date."""
...
def test_semantic_docket_stage_uses_a_positive_govinfo_fallback(
    ...
    revise_citation(document.citations[0], docket_number="24-cv-8760")
    courtlistener = _DocketSearchClient(count=0, results=[])
    govinfo = _GovInfoSearchClient(
        count=1,
        results=[
    ...
    shortlisted = asyncio.run(shortlist_docket_root_metadata_candidates(ready))
    completed = asyncio.run(resolve_docket_root_semantics(shortlisted))
    ...
    assert year.details["validation"]["outcome"] is None
...
def test_docket_identity_defers_when_case_filing_postdates_a_stated_decision() -> None:
    ...
    document = _document(date=None)
    revise_citation(document.citations[0], date=CitationDate(year="2024", month="Jan.", day="4"))
    client = _DocketSearchClient(count=1, results=[_candidate()])
...
def test_docket_identity_resolves_when_the_case_filing_date_is_compatible() -> None:
    """A noncontradictory filing date permits root identity, not order-date proof."""
    document = _document(date=None)
    revise_citation(document.citations[0], date=CitationDate(year="2024", month="Jan.", day="6"))
    client = _DocketSearchClient(count=1, results=[_candidate()])
...
def test_programmatic_docket_identity_defers_nonliteral_docket_forms_to_semantics() -> None:
    ...
    assert root.fields.docket_number == "24-cv-8760"
    assert root.judgement(Question.IDENTITY).outcome == "no_match"
    assert root.found is None
    assert not any("Mellea" in node.made_by for node in root.trace)
...
def test_metadata_shortlist_keeps_every_similar_docket_before_one_model_choice(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A partial response may yield a positive, traced semantic docket selection."""
    document = _document(date=None)
    revise_citation(document.citations[0], docket_number="20 Civ. 6835")
    client = _DocketSearchClient(
        count=21,
        results=[
            _candidate(docket="1:20-cv-06835", docket_id=1),
            _candidate(docket="1:21-cv-06835", docket_id=2),
            _candidate(docket="1:20-cv-06835", docket_id=3) | {"court_id": "nyed"},
        ],
    )
    ...
    async def unchanged_review(record, **kwargs):
        return MelleaDocketCitationReextractionNode(
            ...
            reason="The source extraction is unchanged.",
            depends_on=("cite-0001:locator_identity_resolution",),
        )

    async def choose_first(validation, *, summary, shortlist, **kwargs):
        assert {candidate.candidate_index for candidate in shortlist.candidates} == {1, 2, 3}
        by_index = {candidate.candidate_index: candidate for candidate in summary.candidates}
        assert set(by_index) == {1, 2, 3}
        assert by_index[3].case_name_outcome.value == "match"
        assert by_index[3].year_outcome is None
        assert by_index[3].court_outcome.value == "mismatch"
        return MelleaLocatorCandidateChoiceNode(
            ...
            selected_candidate_index=1,
            rationale="Candidate 1 has the closest ordinary docket presentation and matching case name.",
            depends_on=(summary.node_id,),
        )

    monkeypatch.setattr(docket_roots, "run_mellea_docket_citation_reextraction", unchanged_review)
    monkeypatch.setattr(docket_roots, "run_mellea_docket_metadata_choice", choose_first)
    searched = asyncio.run(search_docket_roots(form_roots(document), client=client))
    unique = asyncio.run(validate_unique_docket_root_identities(searched))
    ambiguous = asyncio.run(resolve_docket_root_ambiguities(unique))
    ...
    shortlist = next(node for node in root.trace if node.stage == "docket_root_metadata_shortlist")
    payload = shortlist.details["validation"]
    assert payload["total_candidate_count"] == 21
    assert payload["returned_candidate_count"] == 3
    assert payload["complete_result_set"] is False
    assert {candidate["candidate_index"] for candidate in payload["candidates"]} == {1, 2, 3}
    assert payload["candidates"][0]["parties"] == ["Smith", "Jones"]
    assert root.judgement(Question.IDENTITY).outcome == "resolved"
    assert root.authority_id == "courtlistener:docket:1"
...
def test_failed_docket_lookup_is_reviewed_once_then_corrected_and_requeued(
    ...
    document = _document(date=None)
    revise_citation(document.citations[0], docket_number="1:24-cv-0876O")
    ...
    root = Document.model_validate(resolved.model_dump(mode="json")).citations[0]

    assert client.calls == [
        ("1:24-cv-0876O", "d", None),
        ("1:24-cv-08760", "d", None),
    ]
    ...
    assert root.authority_id == "courtlistener:docket:44"
    review_updates = [
        update for update in root.field_updates if ":fixture:" not in update.node_id
    ]
    assert {update.field for update in review_updates} == {
        CitationField.DOCKET_NUMBER,
        CitationField.CASE_NAME,
        CitationField.COURT_TEXT,
        CitationField.DATE,
    }
    assert {update.node_id for update in review_updates} == {
        "cite-0001:mellea_docket_citation_reextraction"
    }
    assert root.case_name is not None and root.case_name.text == "Smith v. Jones"
    assert root.fields.court_text == "S.D.N.Y."
...
def test_semantic_docket_stage_confirms_a_nonliteral_court_anchored_record(
    ...
    ready = asyncio.run(resolve_requeued_docket_root_ambiguities(requeued_unique))
    shortlisted = asyncio.run(shortlist_docket_root_metadata_candidates(ready))
    resolved = asyncio.run(resolve_docket_root_semantics(shortlisted))
    root = Document.model_validate(resolved.model_dump(mode="json")).citations[0]
...
def test_semantic_docket_stage_leaves_no_candidate_and_review_limit_unresolved(
    ...
    document = _document()
    client = _DocketSearchClient(count=21, results=[_candidate(docket_id=index) for index in range(20)])
    ...
    async def reject_first_page(validation, *, summary, shortlist, **kwargs):
        return MelleaLocatorCandidateChoiceNode(
            node_id=f"{summary.node_id}:mellea_docket_metadata_choice",
            status=ValidationNodeStatus.SUCCEEDED,
            outcome=MelleaLocatorCandidateChoiceOutcome.NO_MATCH,
            ...
            selected_candidate_index=None,
            rationale="No first-page result represents the stated citation.",
            depends_on=(summary.node_id,),
        )

    monkeypatch.setattr(docket_roots, "run_mellea_docket_citation_reextraction", unchanged_review)
    monkeypatch.setattr(docket_roots, "run_mellea_docket_metadata_choice", reject_first_page)
    searched = asyncio.run(search_docket_roots(form_roots(document), client=client))
    unique = asyncio.run(validate_unique_docket_root_identities(searched))
    ambiguous = asyncio.run(resolve_docket_root_ambiguities(unique))
    reviewed = asyncio.run(review_and_requeue_unresolved_docket_roots(ambiguous, client=client))
    ...
    assert root.authority_id is None
    assert any(
        node.details.get("validation_node_type") == "MelleaLocatorCandidateChoiceNode"
        and node.stage == "docket_root_semantic_resolution"
        for node in root.trace
    )
