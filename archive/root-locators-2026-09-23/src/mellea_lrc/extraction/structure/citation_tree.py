...
def assign_roots(citations: Sequence[CitationRecord]) -> tuple[CitationRecord, ...]:
    ...
    """
    by_id = {item.citation_id: item for item in citations if not item.withdrawn}
    ...
    for item in citations:
        ...
        placed = root is not None and isinstance(root.fields, _ROOT_KINDS)
    ...
    for item in citations:
        if item.withdrawn:
            result.append(item)
            continue
        root_id = assigned[item.citation_id]
        if root_id is None or item.root_id == root_id:
            result.append(item)
            continue
        node = Node(
            node_id=f"root_assignment:{item.citation_id}:{root_id}",
            reads=Reads.RECORD,
            stage="root_assignment",
            made_by=__name__,
            outcome="attached",
            details={"root_id": root_id},
        )
        result.append(assign_root(item, root_id, node, resolves_to=item.resolves_to))
    return tuple(result)
...
def build_citation_tree(document: Document) -> CitationTree:
    ...
    for item in document.active_citations:
        ...
        if assigned_root is not None and isinstance(assigned_root.fields, _ROOT_KINDS):
            root_id = assigned
            # `root_id` is the direct aggregation pointer, while `resolves_to`
            # preserves the citation chain. Use that chain for display depth
            # only when it reaches the same assigned root.
            chain_root, chain_depth = _resolve_root(item, by_id)
            depth = chain_depth if chain_root == assigned else (0 if assigned == item.citation_id else 1)
        else:
            root_id, depth = _resolve_root(item, by_id)
        root = by_id.get(root_id) if root_id else None
        if root is not None and isinstance(root.fields, _ROOT_KINDS):
...
def _is_out_of_scope(item: CitationRecord, root: CitationRecord | None) -> bool:
    ...
    """
    if isinstance(item.fields, _CASE_KINDS):
        return False
    ...
    if not reached_something_else:
        return False
    return not isinstance(root.fields, _CASE_KINDS)
...
def _needs_an_antecedent(item: CitationRecord) -> bool:
    ...
    return item.fields.kind in _REFERRING_KINDS
...
def _pin_cite(item: CitationRecord) -> str | None:
    ...
    pin_cite = getattr(item.fields, "pin_cite", None)
