...
This runs over the document with every citation and admitted case name blanked,
so what it looks at is by definition what nothing else read. A case name standing in that
...
def unread_case_names(text: str, citations: Sequence[CitationRecord]) -> tuple[Span, ...]:
    ...
    for citation in citations:
        # A reviewed name can belong to a locator whose original full span did
        # not reach it. Mask the accepted name as well, without widening that
        # citation across intervening prose or another locator.
        spans = (citation.full_span,)
        if citation.case_name_span is not None:
            spans = (*spans, citation.case_name_span)
        for span in spans:
