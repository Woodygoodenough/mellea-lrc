...
def _of_kind(text: str, kind: CitationKind) -> CitationRecord:
    return next(c for c in _citations(text) if c.fields.kind is kind)
...
def test_the_span_holds_the_pin_cite_the_citation_states(
    ...
    assert citation.fields.pin_cite.text == expected
...
def test_every_kind_spells_the_page_the_same_way(text: str, kind: CitationKind) -> None:
    ...
    assert citation.fields.pin_cite is not None
    assert citation.fields.pin_cite.text[0].isdigit()
    assert citation.pin_cite_span is not None
    start, end = citation.pin_cite_span.start, citation.pin_cite_span.end
    assert text[start:end] == citation.fields.pin_cite.text
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "United States v. Chen Zhi , No. 1:25-cr-00312-RPK (E.D.N.Y. filed Oct. 8, 2025). Id. ¶¶ 26-28.",
            CitationKind.ID,
            "¶¶ 26-28",
        ),
    ],
