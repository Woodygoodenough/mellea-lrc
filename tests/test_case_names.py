...
def test_a_table_of_authorities_entry_keeps_the_name_before_its_leader_dots() -> None:
    """The dots fall on one side of the name or the other, and what follows says which.

        Prev Name ….. 4 This Name , 123 F.3d 4      the dots end the entry above
        This Name ….. 5 123 F.3d 4                  the dots end this entry

    Reading it the first way in both cases left the second with no name at all:
    the start jumped past the dots onto ` 5 `, which has no letters in it.
    """
    text = (
        "Perfect Fit Industries Inc. v. Acme Quilting , 673 F.2d 53 (2 nd Cir. 1982)…………. 6 "
        "Precision Instrument Mfg. Co. v. Automotive Maintenance Machinery Co……….. 5 "
        "324 U.S. 806 (1945)"
    )
    assert _names(text) == [
        "Perfect Fit Industries Inc. v. Acme Quilting",
        "Precision Instrument Mfg. Co. v. Automotive Maintenance Machinery Co",
    ]


def test_a_citation_with_parties_has_a_name() -> None:
    """The parties are characters this document holds, so the name is in it.

    A record with `plaintiff` and `defendant` and no `case_name` has lost the
    place, not the name -- and every reading after extraction matches on
    `case_name.text`, so losing the place loses the name for all of them.
    """
    for text in (
        "Perfect Fit Industries Inc. v. Acme Quilting , 673 F.2d 53 (2 nd Cir. 1982)…………. 6 "
        "Precision Instrument Mfg. Co. v. Automotive Maintenance Machinery Co……….. 5 324 U.S. 806 (1945)",
        "Bell Atl. Corp. v. Twombly , 550 U.S. 544 (2007). The court in Bell at 546 said so.",
    ):
        with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
            document = extract_from_plain_text(text, relaxation=Relaxation.FULL, with_leaves=True)
        for citation in document.citations:
            parties = getattr(citation.stated, "plaintiff", None) or getattr(
                citation.stated, "defendant", None
            )
            assert not (parties and citation.stated.case_name is None), citation.stated.matched_text
